...
// Reads the default streaming height from the "Streaming/Quality" config label.
// "Best Available" (or unset) -> -1 (no cap). "1080p" / "2160p (4K)" -> 1080 / 2160.
int SgYtDlp::defaultStreamHeight() const {
...
int SgYtDlp::heightForFormatId(const QJsonArray& formats, const QString& id) const {
...
bool SgYtDlp::chooseMatchedAvPair(const QJsonArray& formats, int targetH,
    ...
    if (pickFamily({ "mp4","m4v" }, { "m4a","mp4" }, { "mp4a","aac" }, vUrlOut, aUrlOut)) return true;
    if (pickFamily({ "webm" }, { "webm" }, { "opus","vorbis" }, vUrlOut, aUrlOut)) return true;
    return false;
}
...
void SgYtDlp::handleProcessFinished(int exitCode, QProcess::ExitStatus exitStatus) {
    ...
    if (mode == JobMode::FetchingPlaylist) {
        ...
        int targetH = wantId.isEmpty() ? defaultStreamHeight()
                                       : heightForFormatId(formats, wantId);

        QString videoUrl, audioUrl;
        if (chooseMatchedAvPair(formats, targetH, videoUrl, audioUrl)) {
            ...
            if (prog.isEmpty()) prog = bestProgressiveUrl(formats);
            if (!prog.isEmpty())
