...
public slots:
    ...
    void setGullStyle(bool animated);                    // true = GIF, false = white outline
...
private:
    ...
    void recycleGull(Gull& g, bool fromLeft);
    ...
    QVector<QPixmap> m_gullFrames; // animated gull frames (empty => silhouette fallback)
    QVector<int>     m_frameDelays; // per-frame display time (ms) — native gif speed
    int   m_frameIdx = 0;
    qreal m_frameAcc = 0.0;
    bool  m_useGif   = true;       // false => painted white outline (settings choice)
